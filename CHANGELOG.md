...
### 2026-09-06
- **fix(init): nothing sleeps silently.** The two "hold rather than exit" paths in `emergency_shell` slept in an hour-long loop saying nothing, which is indistinguishable from a wedged machine, a powered-off one, or one on the wrong network — and a node between power-on and cluster join has no other way to be seen. `hold(why)` now repeats what it is waiting on every minute, to the console *and* to `/dev/kmsg` so it survives a console nobody had attached yet and shows up in `dmesg` for anyone who gets further. The "no shell" case also says what it means: a build fault, since busybox should be at `/bin/sh`.

- **fix(init): console output is ASCII.** The same reason it changed in stormblock's shell init and in stormuefi: the only console a headless node has is a serial line, which is 7-bit in practice, so an em-dash arrives as two bytes of noise in the middle of the line being read. `steps.rs` and `media.rs` carried them, including the message printed when the local disk holds no slab. Comments keep their typography; nothing prints those.
